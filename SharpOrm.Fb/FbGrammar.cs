...
namespace SharpOrm.Fb
{
    ...
    {
        public FbGrammar(Query query) : base(query, new FbQueryBuilder(query))
        {
        }

        protected override void ConfigureInsertQuery(QueryBase query, IEnumerable<string> columnNames)
        {
            this.ThrowPaginationNotSupported();
            this.ThrowOffsetNotSupported();
            this.ThrowOrderNotSupported();
            this.ThrowJoinNotSupported();

            this.AppendInsertHeader(columnNames);
            this.builder.AddAndReplace(
                query.ToString(),
                '?',
                (count) => this.builder.AddParameter(this.Info.Where.Parameters[count - 1])
            );
        }
        ...
        protected override void ConfigureBulkInsert(IEnumerable<Row> rows)
        {
            ...
            using (var @enum = rows.GetEnumerator())
            {
                ...
                var dbBuilder = this.builder as FbQueryBuilder;
                do
                {
                    this.ConfigureInsert(@enum.Current.Cells, false);
                    dbBuilder.IsBlock = true;
                }
                while (@enum.MoveNext());
            }
        }
        ...
        protected override void ConfigureDelete()
        {
            ...
            this.builder.Add("DELETE FROM ");
            this.builder.Add(this.Info.TableName.GetName(true, this.Info.Config));
        ...
        protected override void ConfigureInsert(IEnumerable<Cell> cells, bool getGeneratedId)
        {
            ...
            if (this.Query.InsertReturnId && getGeneratedId && this.Query.ReturnsInsetionId) this.builder.Add(" RETURNING ID;");
        ...
        protected override void ConfigureCount(Column column)
        {
            ...
            if (safeDistinct)
                this.builder.Add(") AS count"); // Firebird não usa aspas para apelidos de tabela
        }
        ...
        private QueryBuilder WritePagination()
        {
            if (this.Query.Limit is null && this.Query.Offset is null)
                return this.builder;

            if (this.Query.Limit != null && this.Query.Offset == null)
                return this.builder.Add(" ROWS ").Add(this.Query.Limit + 1);

            if (this.Query.Limit == null && this.Query.Offset != null)
                return this.builder.Add(" ROWS ").Add(this.Query.Offset).Add(" TO ").Add(int.MaxValue);

            return this.builder.Add(" ROWS ").Add(this.Query.Offset + 1)
                .Add(" TO ").Add((this.Query.Limit ?? int.MaxValue) + (this.Query.Offset ?? 0));
        }
        ...
        protected override void ConfigureUpdate(IEnumerable<Cell> cells)
        {
            ...
            this.AddRowsAffectedLimit();
        }

        private void AddRowsAffectedLimit()
        {
            if (this.Query.Limit != null)
                this.builder.Add(" ROWS ").Add(this.Query.Limit);
        }
